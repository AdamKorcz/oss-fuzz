...
static void chain_hash_actions(PyObject *h, FuzzedDataProvider &fdp) {
  enum { UPDATE, DIGEST, HEXDIGEST, COPY_DIGEST, ATTRS, NUM_ACTIONS };
  for (int i = 0; fdp.remaining_bytes() > 0 && i < 100; i++) {
    switch (fdp.ConsumeIntegralInRange<int>(0, NUM_ACTIONS - 1)) {
      ...
      case ATTRS: {  // .name, .digest_size, .block_size
...
static void op_shake_chain(PyObject *ctor, FuzzedDataProvider &fdp) {
  ...
  PyRef h = PyObject_CallFunction(ctor, "y#", Y(init));
  CHECK(h);
  enum { UPDATE, DIGEST, COPY_DIGEST, NUM_ACTIONS };
  for (int i = 0; fdp.remaining_bytes() > 0 && i < 100; i++) {
    switch (fdp.ConsumeIntegralInRange<int>(0, NUM_ACTIONS - 1)) {
      case UPDATE: {
        std::string data = fdp.ConsumeBytesAsString(
...
// hmac.compare_digest() against a fuzz-sized buffer derived from
// the same data. Exercises the constant-time comparison code path.
static void op_hmac_compare(FuzzedDataProvider &fdp) {
  size_t pad_size = fdp.ConsumeIntegralInRange<size_t>(1, 128);
  ...
  CHECK(dig);
  std::string padded(pad_size, '\0');
  memcpy(padded.data(), data.data(),
         data.size() < pad_size ? data.size() : pad_size);
  PyRef padobj = PyBytes_FromStringAndSize(Y(padded));
  CHECK(padobj);
