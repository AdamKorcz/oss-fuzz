...
static void op_bytesio(FuzzedDataProvider &fdp) {
  enum { BASIC_RW, READLINE_READINTO, TRUNCATE_WRITE, GETBUFFER, READ1_READINTO1, ITERATION, PEEK_BUFFERED, NUM_TARGETS };
  ...
  switch (target_fn) {
    ...
    case READLINE_READINTO: {
      ...
      }
      {
        PyRef sk = PyObject_CallMethod(bio, "seek", "i", 0);
        if (PyErr_Occurred()) PyErr_Clear();
        Py_ssize_t buf_sz = fdp.ConsumeIntegralInRange<int>(1, 10000);
        PyRef buf = PyByteArray_FromStringAndSize(NULL, buf_sz);
        CHECK(buf);
    ...
    case TRUNCATE_WRITE: {
      ...
      if (PyErr_Occurred()) PyErr_Clear();
      std::string wr_data = fdp.ConsumeBytesAsString(
          fdp.ConsumeIntegralInRange<size_t>(1, std::min(fdp.remaining_bytes() + 1, (size_t)1024)));
      PyRef wr = PyObject_CallMethod(bio, "write", "y#", Y(wr_data));
      if (PyErr_Occurred()) PyErr_Clear();
      PyRef gv = PyObject_CallMethod(bio, "getvalue", NULL);
      if (PyErr_Occurred()) PyErr_Clear();
      PyRef cl = PyObject_CallMethod(bio, "close", NULL);
...
static void op_textiowrapper(FuzzedDataProvider &fdp) {
  ...
  enum { WRITE_MODE, READ_MODE, RECONFIGURE, DETACH, PROPERTIES, NUM_TARGETS };
  int target_fn = fdp.ConsumeIntegralInRange<int>(0, NUM_TARGETS - 1);
  ...
  switch (target_fn) {
    ...
    case RECONFIGURE: {
      ...
      }
      {
        ...
        CHECK(empty);
        PyRef reconf = PyObject_GetAttrString(tw, "reconfigure");
        if (reconf) {
          PyRef r = PyObject_Call(reconf, empty, kw);
        }
        if (PyErr_Occurred()) PyErr_Clear();
      }
      {
...
static void op_buffered_io(FuzzedDataProvider &fdp) {
  enum { BUFFERED_READER, BUFFERED_WRITER, BUFFERED_RANDOM, BUFFERED_RWPAIR, NUM_TARGETS };
  int target_fn = fdp.ConsumeIntegralInRange<int>(0, NUM_TARGETS - 1);
  if (fdp.remaining_bytes() == 0) return;
  size_t data_len = fdp.ConsumeIntegralInRange<size_t>(
      1, std::min(fdp.remaining_bytes(), (size_t)10000));
...
static void op_fileio(FuzzedDataProvider &fdp) {
  enum { READ_FILE, WRITE_FILE, READWRITE_FILE, NUM_TARGETS };
  int target_fn = fdp.ConsumeIntegralInRange<int>(0, NUM_TARGETS - 1);
  if (fdp.remaining_bytes() == 0) return;
  size_t data_len = fdp.ConsumeIntegralInRange<size_t>(
      1, std::min(fdp.remaining_bytes(), (size_t)10000));
...
static void op_io_open(FuzzedDataProvider &fdp) {
  enum { READ_TEXT, READ_BINARY, WRITE_TEXT, WRITE_BINARY, NUM_TARGETS };
  int target_fn = fdp.ConsumeIntegralInRange<int>(0, NUM_TARGETS - 1);
  int str_enc = fdp.ConsumeIntegralInRange<int>(0, 3);
  if (fdp.remaining_bytes() == 0) return;
  size_t data_len = fdp.ConsumeIntegralInRange<size_t>(
...
static void op_stringio(FuzzedDataProvider &fdp) {
  int str_enc = fdp.ConsumeIntegralInRange<int>(0, 3);
  enum { READLINE_READLINES, READLINES_ONLY, NUM_TARGETS };
  int target_fn = fdp.ConsumeIntegralInRange<int>(0, NUM_TARGETS - 1);
  if (fdp.remaining_bytes() == 0) return;
  size_t data_len = fdp.ConsumeIntegralInRange<size_t>(
      1, std::min(fdp.remaining_bytes(), (size_t)10000));
  ...
  if (target_fn == READLINE_READLINES) {
